...
void fast_9_old::operator()(const Image<byte>& i, vector<ImageRef>& c, unsigned int n) const 
{
	vector<ImageRef> ct;
	vector<int> sc;
...
void fast_9::operator()(const Image<byte>& i, vector<ImageRef>& c, unsigned int n) const 
{
	fast_corner_detect_9_nonmax(i, c, static_cast<int>(n));
}

void fast_12::operator()(const Image<byte>& i, vector<ImageRef>& c, unsigned int n) const 
{
	vector<ImageRef> cs;
